...
var _ = Describe("Account", func() {
	...
	BeforeEach(func() {
		...
		wallet, err = multi.CreateNewWallet("0000", 0)
		Expect(err).To(BeNil())
	...
	Describe("GetAccounts(requiredConfirmations int32) (string, error)", func() {
	...
	Describe("GetAccount(accountNumber int32, requiredConfirmations int32) (*Account, error)", func() {
	...
	Describe("GetAccountBalance(accountNumber int32, requiredConfirmations int32) (*Balance, error)", func() {
		Context("when called", func() {
	...
	Describe("SpendableForAccount(account int32, requiredConfirmations int32) (int64, error)", func() {
		Context("when called", func() {
			It("should return the spendable balance", func() {
				
				result, err := wallet.SpendableForAccount(0, 0)
				By("returning a nil error")
				Expect(err).To(BeNil())
				By("returning the expected balance")
	...
	Describe("NextAccount(accountName string, privPass []byte) (int32, error)", func() {
		Context("when called", func() {
			It("should return the next account number", func() {
				
				accountNumber, err := wallet.NextAccount("account 1", []byte("0000"))
				By("returning a nil error")
				Expect(err).To(BeNil())
		...
		Context("when called with a wrong password", func () {
			It("should failed", func() {
				_, err := wallet.NextAccount("account 1", []byte("1902"))
				By("returning a non nil error")
				Expect(err).ToNot(BeNil())
	...
	Describe("RenameAccount(accountNumber int32, newName string) error", func() {
	...
	Describe("AccountName(accountNumber int32) string", func() {
		Context("when called", func() {
	...
	Describe("AccountNameRaw(accountNumber uint32) (string, error)", func() {
	...
	Describe("AccountNumber(accountName string) (uint32, error)", func() {
		Context("when called", func() {
	...
	Describe("HDPathForAccount(accountNumber int32) (string, error)", func() {
		Context("when called", func() {
