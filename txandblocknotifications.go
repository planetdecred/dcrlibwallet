...
func (mw *MultiWallet) listenForTransactions(lw *LibWallet) {
	...
	for {
		...
		for _, transaction := range v.UnminedTransactions {
			...
			if err != nil {
				log.Errorf("[%d] Error ntfn parse tx: %v", lw.wallet.ID, err)
				return
			}

			overwritten, err := lw.txDB.SaveOrUpdate(&Transaction{}, tempTransaction)
			if err != nil {
				log.Errorf("[%d] New Tx save err: %v", lw.wallet.ID, err)
				return
			}

			if !overwritten {
				log.Infof("[%d] New Transaction %s", lw.wallet.ID, tempTransaction.Hash)
		...
		for _, block := range v.AttachedBlocks {
			...
			for _, transaction := range block.Transactions {
				...
				if err != nil {
					log.Errorf("[%d] Incoming block replace tx error :%v", lw.wallet.ID, err)
					return
				}
				mw.publishTransactionConfirmed(lw.wallet.ID, transaction.Hash.String(), int32(block.Header.Height))
			}
		}
	}
}
